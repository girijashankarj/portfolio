...
const VISIBLE_PROJECTS = [...PROJECTS].sort(
...
function ProjectCategoryCard({ categoryId, categoryLabel, categorySummary, projects }: ProjectCategoryCardProps) {
  if (projects.length === 0) return null

  return (
    <Reveal>
      <div className="card" data-category={categoryId}>
        <h3 style={{ marginTop: 0, marginBottom: categorySummary ? '0.35rem' : '1rem' }}>{categoryLabel}</h3>
        {categorySummary && (
          <p style={{ margin: '0 0 1rem', fontSize: '0.85rem', color: 'var(--muted)', lineHeight: 1.45 }}>
            {categorySummary}
          </p>
        )}
  ...
  return (
    <section id="projects" className="section section-band" itemScope itemType="https://schema.org/ItemList">
      <div className="container">
        ...
        <p className="section-kicker reveal">Work grouped by what it proves: agents, architecture, automation, and shipped products.</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 340px), 1fr))', gap: '1.5rem', alignItems: 'stretch' }}>
          {activeCategory === 'all'
            ...
            : (
              <ProjectCategoryCard
                ...
                categorySummary={PROJECT_CATEGORIES.find((c) => c.id === activeCategory)?.summary}
