        ...
        <ol className="list list-ordered">
          {projects.map((project) => (
            <li
              ...
              style={{
                marginBottom: '1rem',
                padding: project.featured ? '0.85rem' : '0.5rem',
                borderRadius: '8px',
                border: project.featured ? '1px solid var(--accent)' : '1px solid transparent',
                background: project.featured ? 'var(--card-strong)' : 'transparent',
                boxShadow: project.featured ? '0 0 0 1px var(--border)' : 'none',
              ...
              onMouseLeave={(e) => {
                e.currentTarget.style.backgroundColor = project.featured ? 'var(--card-strong)' : 'transparent'
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                <span style={{ fontWeight: 700 }}>{project.title}</span>
                {project.featured && (
                  <span className="chip active" style={{ fontSize: '0.7rem', padding: '0.15rem 0.45rem' }}>
                    Featured
                  </span>
                )}
              </div>
              <span style={{ display: 'inline-flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.25rem' }}>
                <a
                  ...
                  style={{ color: 'var(--accent)', fontSize: '0.9rem', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}
                >
              ...
              {project.description && (
                <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.9rem', color: 'var(--muted)', lineHeight: '1.5' }}>
                  {project.description}
                </p>
              )}
              {project.technologies && project.technologies.length > 0 && (
                <div style={{ marginTop: '0.5rem', display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                  {project.technologies.map((tech) => (
                    <span key={tech} className="chip" style={{ padding: '0.2rem 0.5rem', borderRadius: '4px', fontSize: '0.75rem', background: 'var(--pill-bg-blue)', border: '1px solid var(--border)', color: 'var(--text)' }}>
                      {tech}
                    </span>
                  ))}
                </div>
      ...
      <div className="container">
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'clamp(0.4rem, 2vw, 0.6rem)', marginBottom: 'clamp(1.5rem, 4vw, 2rem)', justifyContent: 'center' }}>
          {PROJECT_CATEGORIES.map((category) => (
            <button key={category.id} onClick={() => dispatch(setActiveCategory(category.id))} className={`chip ${activeCategory === category.id ? 'active' : ''}`}>
              {category.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: '1.5rem', alignItems: 'stretch' }}>
          {activeCategory === 'all' ? (
            <>
              <ProjectCategoryCard categoryId="dev-tools" categoryLabel="Developer Tools & Config" projects={PROJECTS_BY_CATEGORY['dev-tools']} />
              <ProjectCategoryCard categoryId="ai-ml" categoryLabel="AI/ML Projects" projects={PROJECTS_BY_CATEGORY['ai-ml']} />
              <ProjectCategoryCard categoryId="react-web" categoryLabel="React/Web Projects" projects={PROJECTS_BY_CATEGORY['react-web']} />
              <ProjectCategoryCard categoryId="fullstack" categoryLabel="Full Stack Applications" projects={PROJECTS_BY_CATEGORY['fullstack']} />
              <ProjectCategoryCard categoryId="javascript" categoryLabel="JavaScript Projects" projects={PROJECTS_BY_CATEGORY['javascript']} />
            </>
          ) : (
